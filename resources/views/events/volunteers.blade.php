...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden p-4 sm:p-6 mb-6">
    ...
    <div class="mt-6 grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-gray-50 rounded-lg p-4 text-center">
            <p class="text-3xl font-bold text-gray-800">{{ $volunteers->total() }}</p>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
        ...
        <span class="bg-purple-100 text-purple-800 text-xs px-2.5 py-0.5 rounded-full font-medium">{{ $volunteers->total() }} volunteers</span>
    </div>
            ...
            <div class="border border-gray-200 rounded-xl p-4 hover:shadow-md transition bg-white">
                <div class="flex items-start justify-between">
                    ...
                    <form action="{{ route('events.volunteers.remove', ['eventId' => $event->id, 'userId' => $volunteer->id]) }}" method="POST"
                        onsubmit="event.preventDefault(); confirmRemoveVolunteer('{{ route('events.volunteers.remove', ['eventId' => $event->id, 'userId' => $volunteer->id]) }}', '{{ str_replace(["\\", "'"], ["\\\\", "\\'"], $volunteer->name) }}')">
                        @csrf
                        @method('DELETE')
                ...
                <div class="mt-3 pt-3 border-t">
                    ...
                    <div class="mt-3 pt-2 border-t border-gray-100">
                        ...
                        <div class="mt-3 flex gap-2">
                            ...
                            @endif
                            @if($status !== 'absent')
                            <button type="button" onclick="openAbsentModal({{ $volunteer->id }}, '{{ str_replace(["\\", "'"], ["\\\\", "\\'"], $volunteer->name) }}')" class="flex-1 bg-red-100 hover:bg-red-200 text-red-700 text-sm font-medium px-3 py-2.5 rounded-lg transition min-h-[44px]" title="Mark as Absent">
                                ✗ Absent
                            </button>
                            @endif
                        </div>
...
<script>
    ...
    function confirmRemoveVolunteer(actionUrl, volunteerName) {
        ...
        modalBody.innerHTML = `
                ...
                <p class="text-gray-600 mb-4">Are you sure you want to remove ${htmlEscape(volunteerName)} from this event?</p>
    ...
    function openAbsentModal(userId, userName) {
        ...
        modalBody.innerHTML = `
                <h3 class="text-lg font-bold text-gray-800 mb-4">Mark ${htmlEscape(userName)} as Absent</h3>
